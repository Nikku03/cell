...
A pipeline that simulates Syn3A's metabolic network stochastically (Gillespie chemistry on the Luthey-Schulten 356-reaction SBML, plus protein folding, plus 24 manually-encoded protein complexes, plus transport rules) and runs an in-silico knockout for every gene to decide whether each knockout would kill the cell. The decision uses a "vote of seven" detector framework — three detectors look at simulator outputs (metabolite pool changes, reaction firing patterns, parallel-pathway redundancy) and four use biological knowledge sources (protein complex membership, gene annotations). All 455 knockout simulations run in about 50 minutes on a 4-core laptop.

What does NOT run during the production sweep, despite living in the codebase or the docs of related work: transcription, translation, mRNA degradation, protein degradation (built but unwired in `cell_sim/layer3_reactions/gene_expression.py`), regulation, DNA replication, cell division, biomass accumulation, and spatial structure. So this is metabolic-network simulation with complex assembly, not whole-cell simulation. See `RESULTS.md` for the scope-coverage table and what that means for credit assignment between the simulator and the knowledge-based detector priors.
