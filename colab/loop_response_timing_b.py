"""Loop 191b. What sets the TIMING of a response -- with loop 191's three defects corrected.
...
-> outputs/loop_response_timing_b.json
"""
import gzip
import json
import os
...
SP = Path(os.environ.get("CELL_SCRATCH",
                         ...
                         "scratchpad"))
GRTC = SP / "grtc"
OUT = Path(os.environ.get("CELL_OUT", "outputs")) / "loop_response_timing_b.json"
...
def main():
    ...
    grid = np.array([t for t in allt if set(REPS) <= set(comp[t])], dtype=float)
    kept = {comp[int(t)] for t in grid}
    say(f"     retained grid ({len(grid)} points): {[int(x) for x in grid]}")
    ...
    peak_lfc = np.nanmax(np.abs(M), axis=0)
    ...
    th = np.array([half_time(grid, M[:, j]) for j in range(M.shape[1])])
    th_a = np.array([half_time(grid, Ma[:, j]) for j in range(Ma.shape[1])])
    th_b = np.array([half_time(grid, Mb[:, j]) for j in range(Mb.shape[1])])
    ...
    say(f"     {n_resp:,} responders at |log2 FC| >= {MIN_LFC} with the direction agreeing across "
