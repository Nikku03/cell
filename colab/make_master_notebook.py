...
cells = [
    ...
    code("from google.colab import drive; drive.mount('/content/drive')",
         ...
         "rx  = first(f'{D}/**/reactome*.txt', f'{D}/**/reactome*.gmt', f'{D}/**/*eactome*.gmt',",
         "            f'{D}/**/ReactomePathways*.txt', f'{D}/**/*eactome*Pathways*.txt')",
         "if not rx:                                                 # not on Drive -> fetch canonical GMT (~0.3 MB)",
         "    try:",
    ...
    code("# train once, reuse — but RETRAIN when the available feature set CHANGED (e.g. ESM now loads, or a new",
         ...
         "def _saved_feats(rel):",
         "    vj = 'outputs/orphan/signal_combiner_validation.json' if rel=='ppi' else f'outputs/orphan/signal_combiner_{rel}_validation.json'",
         "    return set(json.load(open(vj)).get('features_all', [])) if os.path.exists(vj) else None",
         "want = _env_feats()",
         "need = []",
         "for rel in ('ppi','reg','sig'):",
         "    pkl = 'outputs/orphan/signal_combiner.pkl' if rel=='ppi' else f'outputs/orphan/signal_combiner_{rel}.pkl'",
         "    saved = _saved_feats(rel)",
         "    if os.environ.get('FORCE_RETRAIN') or not os.path.exists(pkl) or saved is None or (want - saved):",
         "        need.append(rel)",
         "if need:",
         "    newish = sorted(want - (_saved_feats('ppi') or set()))",
