...
CELLS = [
    ...
    code(
        ...
        "os.environ['CF_CKPT'] = '500,1500,3000,6000,10000'",
        "# 0 = every held-out scorable perturbation (~419), not a 40-row subsample. Measured on the first",
        "# A100 run: the 95% CI half-width on recall is 0.070 at n=40 and 0.021 at n=419. At n=40 the curve",
        "# came back non-monotonic (0.1573 -> 0.0890 -> 0.2531) purely from sampling noise, and an ablation",
        "# table whose rows differ by 0.02-0.05 would have been unreadable. Eval is forward-only and cheap.",
        "os.environ['CF_NEVAL'] = '0'",
        "",
    ...
    code(
        ...
        "os.environ['CF_NEVAL'] = '0'      # all held-out perturbations; see the note in the curve cell",
