...
code("# [2b] fetch the raw data build_cell_complete.py needs (the big files are NOT in the repo).",
   ...
   "# --- RESTORE from Drive first (fast local copy) so we don't re-download ~4 GB of slow/WAF-prone files ---",
   "if IN_COLAB:",
   "    try:",
   ...
   "        print('restore <-', fn, f'({os.path.getsize(s)//1048576} MB)'); shutil.copy2(s,t); n+=1",
   "    return n",
   "DV='/content/drive/MyDrive/virtual_cell_data'; CM='/content/drive/MyDrive/cell_model'",
   "n_raw=_restore(f'{DV}/human_raw', H)                    # raw data -> data dir (bioplex, DepMap, Perturb-seq, ...)",
   "# reusable EXPENSIVE outputs (skip Model-2 census + DepMap/backbone recompute); NOT the final products",
   "REUSE={'celltype_expression.csv','celltype_masters.json','integrated_cell_human.csv','depmap_essentiality.csv',",
   "       'depmap_codep.json','depmap_sl.json','predicted_essentiality.csv','biomarkers.json','literature_counts.json'}",
   "n_out=_restore(CM, OUT, only=REUSE) + _restore(f'{DV}/cell_build', OUT, only=REUSE)",
   "# CatPred kinetics (the ~30-min GPU run): restore its outputs so we can skip re-running it entirely",
   "n_cat=_restore(f'{DV}/catpred_kinetics', OUT)",
   "if os.path.exists(f'{OUT}/catpred_kcat_output.csv'): os.environ.setdefault('CATPRED_KCAT_OUT', os.path.abspath(f'{OUT}/catpred_kcat_output.csv'))",
   "if os.path.exists(f'{OUT}/catpred_km_output.csv'):   os.environ.setdefault('CATPRED_KM_OUT',   os.path.abspath(f'{OUT}/catpred_km_output.csv'))",
   "if os.path.exists(f'{OUT}/catpred_kinetics.json'): print('  CatPred: restored predictions from Drive -> GPU inference will be skipped')",
   "print(f'Drive restore: {n_raw} raw file(s) + {n_out} precomputed output(s) + {n_cat} CatPred file(s)')",
   "import glob as _glob",
   "_a4=[p for p in _glob.glob(f'{DV}/**/archs4*.h5',recursive=True)+_glob.glob(f'{DV}/**/*human_gene*.h5',recursive=True) if not p.endswith('.part') and os.path.getsize(p)>1e8]",
   "_pax=[p for p in _glob.glob(f'{DV}/**/*paxdb*',recursive=True)+_glob.glob(f'{DV}/**/*WHOLE_ORGANISM*integrated*',recursive=True) if os.path.getsize(p)>1000]",
   "print('  ARCHS4/GEO h5:', (f'{_a4[0]} ({os.path.getsize(_a4[0])/1e9:.1f} GB)' if _a4 else 'NOT FOUND under virtual_cell_data/ -> coexpr/#1 will skip'))",
   "print('  PaxDb file   :', (f'{_pax[0]}' if _pax else 'NOT FOUND under virtual_cell_data/ -> concentration uses expression-calibration only'))",
   "if _a4: os.environ.setdefault('ARCHS4_H5', _a4[0]); os.environ.setdefault('COEXPR_H5', _a4[0])",
   "if _pax: os.environ.setdefault('PAXDB_FILE', _pax[0])",
   "SRC={",
   ...
   "    # (4) NicheNet ligand->target matrix (R .rds) -> ligand signaling layer merged into the model + query engine",
   ...
   "    _sp.run([sys.executable,'colab/compute_nichenet.py'])                   # -> nichenet_ligand_targets.json (merged by build_cell_complete; queryable ligand->target signaling)",
   "# (5) Phase-1 ARCHS4 co-expression network -> the 8th independent lens. ARCHS4 == the GEO RNA-seq",
   "# compendium (~59 GB h5). It is STAGED to local SSD first (h5py random reads over the Drive FUSE",
   "# mount are unusably slow); coexpr / context-networks / #1 then all read the fast local copy.",
   "WITH_COEXPR=os.environ.get('WITH_COEXPR','1')=='1'",
   "if WITH_COEXPR:",
   "    !pip -q install h5py",
   "    _sp.run([sys.executable,'colab/stage_archs4.py'])                       # copy ARCHS4/GEO 59 GB -> local SSD (one-time; fast h5py reads)",
   "    _sp.run([sys.executable,'colab/compute_coexpr.py'])                     # -> coexpr_neighbors.json (reads staged local ARCHS4)",
   "    _sp.run([sys.executable,'colab/compute_context_networks.py'])           # -> context_networks.json (per-condition co-expression from ARCHS4)",
   ...
   "# === STRUCTURE + DOMAIN lenses for dark genes (Foldseek over AlphaFold + Pfam/InterPro) ===",
   "WITH_STRUCTURE=os.environ.get('WITH_STRUCTURE','1')=='1'",
   "if WITH_STRUCTURE:",
   "    # UniProt Pfam/InterPro domains per gene (small)",
   "    _get('uniprot_domains.tsv', os.environ.get('UNIPROT_DOMAINS_URL','https://rest.uniprot.org/uniprotkb/stream?query=organism_id:9606+AND+reviewed:true&fields=accession,gene_primary,xref_pfam,xref_interpro&format=tsv'))",
   "    _sp.run([sys.executable,'colab/compute_domains.py'])                    # -> domain_function.json",
   "    # Foldseek static binary + AlphaFold human proteome (~few GB) for structural function transfer",
   "    if not shutil.which('foldseek'):",
   "        try:",
   "            urllib.request.urlretrieve('https://mmseqs.com/foldseek/foldseek-linux-avx2.tar.gz','/tmp/fs.tgz')",
   "            _sp.run(['tar','xzf','/tmp/fs.tgz','-C','/usr/local'],check=False); os.environ['PATH']+=':/usr/local/foldseek/bin'",
   "        except Exception as e: print('foldseek install skipped:',e)",
   "    AF=f'{H}/alphafold'",
   ...
   "                with _gz.open(gzf,'rb') as a,open(gzf[:-3],'wb') as b: shutil.copyfileobj(a,b)",
   "    _sp.run([sys.executable,'colab/compute_structure.py'])                  # -> structure_function.json",
    "    # PaxDb protein abundance (quantitative 'how much' layer). The web download 403s and is NOT used;",
   "    # compute_abundance loads the PaxDb file from your Drive (virtual_cell_data/). Only try the web if a working PAXDB_URL is set.",
   "    if os.environ.get('PAXDB_URL'): _get('paxdb_human.txt', os.environ['PAXDB_URL'])",
   "    _sp.run([sys.executable,'colab/compute_abundance.py'])                   # -> paxdb_abundance.json (reads Drive PaxDb; prints which file it used)",
   "    if not os.path.exists(f'{H}/corum.txt') and _get('corum.zip', os.environ.get('CORUM_URL','https://mips.helmholtz-muenchen.de/corum/download/releases/current/allComplexes.txt.zip')):",
   "        import zipfile as _zf",
   "        try:",
   "            with _zf.ZipFile(f'{H}/corum.zip') as z:",
   "                nm=[n for n in z.namelist() if n.endswith('.txt')][0]; z.extract(nm,H); os.rename(f'{H}/{nm}', f'{H}/corum.txt')",
   "        except Exception as e: print('corum unzip skipped:', e)",
...
code("import cellxgene_census, scipy.sparse as sp, traceback",
   ...
   "_SKIP_CENSUS = os.path.exists(f'{OUT}/celltype_masters.json') and os.path.exists(f'{OUT}/celltype_expression.csv')",
   "if _SKIP_CENSUS: print('Model 2: reusing restored celltype_masters.json + celltype_expression.csv from Drive (skipping ~15 min census)')",
   "try:",
   "    if _SKIP_CENSUS: raise RuntimeError('SKIP_CENSUS')   # keep restored outputs; masters cell guards on cell_means",
   ...
   "    if 'SKIP_CENSUS' not in str(_e): traceback.print_exc(); print('Model 2 census unavailable -> curated masters fallback')"),
code("masters={}",
...
code("# === LOAD GEO / ARCHS4 (the ~59 GB expression compendium): find it, validate it as HDF5, READ-TEST it, and wire it ===",
   "import glob as _g, os",
   "_cands=[p for p in _g.glob('/content/drive/MyDrive/virtual_cell_data/**/*.h5',recursive=True) if not p.endswith('.part') and os.path.getsize(p)>1e8]",
   "if not _cands:",
   "    print('GEO/ARCHS4: no complete *.h5 found under virtual_cell_data/ -> co-expression + context-networks + #1 condition-multipliers WILL SKIP')",
   "else:",
   "    _a4=max(_cands,key=os.path.getsize); print(f'GEO/ARCHS4 file: {_a4} ({os.path.getsize(_a4)/1e9:.1f} GB)')",
   "    try:",
   "        import h5py, numpy as _np, time as _t",
   "        _t0=_t.time(); _h=h5py.File(_a4,'r')",
   "        _dk=('data/expression' if 'data/expression' in _h else 'expression'); _gk=next((k for k in ['meta/genes/symbol','meta/genes/genes','meta/genes/gene_symbol'] if k in _h),None)",
   "        _ds=_h[_dk]; print(f'  datasets: {_dk} {_ds.shape} {_ds.dtype}; genes-key={_gk}')",
   "        _sl=_ds[:min(50,_ds.shape[0]), :min(50,_ds.shape[1])]   # READ-TEST: actually pull a slice (catches present-but-unreadable Drive files)",
   "        print(f'  read-test OK: pulled {_sl.shape} in {_t.time()-_t0:.1f}s (sum={float(_np.asarray(_sl).sum()):.3g})'); _h.close()",
   "        os.environ['ARCHS4_H5']=_a4; os.environ['COEXPR_H5']=_a4; print('  -> wired ARCHS4_H5 + COEXPR_H5')",
   "    except Exception as _e:",
   "        print('  !! GEO/ARCHS4 READ FAILED:',repr(_e)[:160]); print('     file is present but not usable (truncated / .part / Drive stall) -> re-upload a complete archs4_human_gene.h5')"),
code("# === SANITY CHECK: verify the whole pipeline is connected (scripts parse, imports resolve, order correct, packages, data) before the long run ===",
   "import subprocess, sys",
   "subprocess.run([sys.executable,'colab/sanity_check.py'])"),
code("# build with hard failure if anything is missing (so we never silently ship the old committed HTML)",
   ...
   "# PROPAGATION THERAPY: a self-spreading cure (sim) + real self-amplifying ligand-receptor circuits (needs full build)",
   "rsp=subprocess.run([sys.executable,'colab/simulate_propagation.py'],capture_output=True,text=True); print(rsp.stdout[-700:])",
   "rpc=subprocess.run([sys.executable,'colab/build_propagation_therapy.py'],capture_output=True,text=True); print(rpc.stdout[-700:])",
   ...
   "# Calibration/validation: measure the accuracy of each answer type against gold standards",
   ...
   "# Extra MEASURED human kinetics (commercial-safe): CatPred-DB (MIT) + UniProt (CC-BY) -> more kcat/Km anchors + a NEW measured-Ki layer",
   "rke=subprocess.run([sys.executable,'colab/fetch_kinetics_extra.py'],capture_output=True,text=True); print(rke.stdout[-500:])",
   "# Kinetics (imputed, honest): measured anchors (SABIO/eHMN/BRENDA + extras, with conditions) + family prior + network propagation x PaxDb abundance",
   "rkin=subprocess.run([sys.executable,'colab/compute_kinetics.py'],capture_output=True,text=True); print(rkin.stdout[-900:])",
   "# (#1) Fit per-gene condition multipliers from ARCHS4 (data-driven master-program magnitudes; replaces flat COND_FOLD). Needs the ARCHS4 human gene h5; skips gracefully if absent.",
   "rfm=subprocess.run([sys.executable,'colab/fit_condition_multipliers.py'],capture_output=True,text=True); print(rfm.stdout[-900:])",
   "# Concentration (E. coli method ported to human): baseline (PaxDb, else expression-calibrated) x fitted condition multiplier x cell-type factor -> absolute nM + family-Kd TF occupancy + velocity",
   ...
   "# Metabolites as first-class species (Problem 5): producers/consumers/compartments + flux turnover + intracellular concentration",
   "rmet=subprocess.run([sys.executable,'colab/compute_metabolites.py'],capture_output=True,text=True); print(rmet.stdout[-600:])",
   "# MEASURED absolute metabolite concentrations (Park 2016, ~121 uM values, commercial-safe facts) -> measured-literature anchors",
   "rmcc=subprocess.run([sys.executable,'colab/fetch_metabolite_conc.py'],capture_output=True,text=True); print(rmcc.stdout[-400:])",
   "# Metabolite concentration: MEASURED (Park2016) + ions (physiological constants) + central metabolites (thermodynamic NET analysis)",
   ...
   "if os.environ.get('RUN_CATPRED','0')=='1' and not os.path.exists(f'{OUT}/catpred_kinetics.json'):   # OPTIONAL CatPred inference (GPU, e.g. L4). Skipped if predictions were restored from Drive.",
   "    import subprocess as _cp, glob as _g",
   ...
   "    # CatPred's `pip install -e .` only pulls the thin service deps (fastapi/pydantic/pandas); the ML runtime",
   "    # deps live in environment.yml's pip: section and are NOT installed. Install them unconditionally (runs even",
   "    # when CatPred is already cloned): ESM embeddings, rdkit, transformers, etc.",
   "    for _pkg in ['ipdb','fair-esm','progres','transformers','descriptastorus','sentencepiece','seaborn','rotary_embedding_torch==0.6.5','typed-argument-parser','pandas-flavor','tensorboardX']:",
   "        _cp.run([sys.executable,'-m','pip','install','-q',_pkg])   # per-package so one failure can't abort the rest; rdkit already ships in Colab",
   ...
   "    def _find_ckpt(param):",
   "        # the checkpoint_dir must contain .pt files (pretrained models from capsule_data_update.tar.gz).",
   "        # batch_demo.ipynb uses data/pretrained/production/{param}; find the {param} dir that actually has a .pt,",
   "        # preferring production/pretrained, so we never hand it the CatPred-DB dataset dir (no .pt -> error).",
   "        cands=[d for d in _g.glob('CatPred/**/'+param, recursive=True) if os.path.isdir(d) and _g.glob(os.path.join(d,'**','*.pt'), recursive=True)]",
   "        cands.sort(key=lambda d:(('production' not in d),('pretrained' not in d), len(d)))",
   "        ck=cands[0] if cands else next((d for d in _g.glob('CatPred/**/'+param, recursive=True) if os.path.isdir(d)), param)",
   "        return ck[len('CatPred/'):] if ck.startswith('CatPred/') else ck   # path is relative to CatPred/ (demo runs there)",
   "    import shutil as _sh",
   "    for param in ['kcat','km']:",
   "        ck=_find_ckpt(param); print(f'  {param} checkpoint_dir -> {ck}')",
   "        r=_cp.run(f'cd CatPred && python demo_run.py --parameter {param} --input_file {inp} --checkpoint_dir {ck} --use_gpu', shell=True, capture_output=True, text=True)",
   ...
   "# REFINE kcat: measured > flux-deconvolved (v/[E]*(Km+[S])/[S]) > CatPred(bias-calibrated)+ECprior ensemble + Km->saturation",
   "rrk=subprocess.run([sys.executable,'colab/refine_kinetics.py'],capture_output=True,text=True); print(rrk.stdout[-700:])",
   "# Dynamic FBA (Problem 1, Born-Oppenheimer): culture-scale metabolite/biomass trajectories over hours, no kinetic ODEs",
   "rdf=subprocess.run([sys.executable,'colab/compute_dfba.py'],capture_output=True,text=True); print(rdf.stdout[-600:])",
   "# Attractor states (Problem 1): stable states of the regulatory network + single-TF transitions between basins",
   "rat=subprocess.run([sys.executable,'colab/compute_attractors.py'],capture_output=True,text=True); print(rat.stdout[-800:])",
   "# Dynamics (Problem 1, turnover level): measured protein+mRNA half-lives (CC-BY) -> per-gene response times, synthesis rates, cell-cycle phase",
   "rdd=subprocess.run([sys.executable,'colab/fetch_dynamics_data.py'],capture_output=True,text=True); print(rdd.stdout[-400:])",
   "rdy=subprocess.run([sys.executable,'colab/compute_dynamics.py'],capture_output=True,text=True); print(rdy.stdout[-600:])",
   "# Time resolution (Problem 1): the relaxation equation dx/dt=k(x*-x), validated vs a real EGF time course (GSE6783)",
   "rtc=subprocess.run([sys.executable,'colab/fetch_timecourse.py'],capture_output=True,text=True); print(rtc.stdout[-400:])",
   "rtr=subprocess.run([sys.executable,'colab/compute_timeresolved.py'],capture_output=True,text=True); print(rtr.stdout[-700:])",
   "# Learned time-predictor: does ML on our features beat the mechanistic equation? (gradient boosting; transformer when multi-perturbation data is added)",
   "rtm=subprocess.run([sys.executable,'colab/compute_timeresolved_ml.py'],capture_output=True,text=True); print(rtm.stdout[-600:])",
   "# Space (Problem 2): HPA multi-compartment localization + Human-GEM compartment-transport topology + PPI co-localization",
   "rsp=subprocess.run([sys.executable,'colab/compute_space.py'],capture_output=True,text=True); print(rsp.stdout[-600:])",
   "# Variants (Problem 6): ClinVar pathogenic burden + constraint + essentiality -> vulnerability + understudied disease-gene candidates",
   "rvr=subprocess.run([sys.executable,'colab/compute_variants.py'],capture_output=True,text=True); print(rvr.stdout[-600:])",
   ...
   "    # persist CatPred outputs + fetched measured/dynamics data to virtual_cell_data/ so a future runtime",
   "    # restores them and SKIPS the ~30-min GPU run and the re-fetches (Park/Mathieson/RNADecayCafe/CatPred-DB).",
   "    _ck='/content/drive/MyDrive/virtual_cell_data/catpred_kinetics'; os.makedirs(_ck, exist_ok=True)",
   "    for f in ['catpred_input.csv','catpred_kcat_output.csv','catpred_km_output.csv','catpred_kinetics.json','kinetics_refined.json','davidi_kcat.json',",
   "              'kinetics_measured_extra.tsv','ki_measured.tsv','metabolite_conc_measured.tsv',",
   "              'protein_halflife.tsv','mrna_halflife.tsv','cellcycle_phase.tsv','dynamics.json',",
   "              'timecourse_egf.tsv','timeresolved.json','timeresolved_ml.json']:",
   "        p=f'{OUT}/{f}'",
   "        if os.path.exists(p): shutil.copy(p, f'{_ck}/{f}')",
   "    print('copied all 3 apps + data + CatPred kinetics + measured/dynamics extras to Drive:', PROJ)"),
code("# [OUTPUT] bundle the WHOLE model into a single JSON you can download and hand back for evaluation",
   ...
   "    for f in ['convergence.json','reasoning.json','coexpr_neighbors.json','causal_reg.json','cell_instances.json','ncrna_targets.json','conditions.json','condition_multipliers.json','kinetics.json','concentration.json','flux.json','metabolites.json','dfba.json','attractors.json','space.json','variants.json','invivo_kcat.json','kinetics_refined.json','catpred_kinetics.json','nichenet_ligand_targets.json','structure_function.json','domain_function.json','paxdb_abundance.json','context_networks.json','go_terms.json','calibration.json','cell_model_summary.json']:",
