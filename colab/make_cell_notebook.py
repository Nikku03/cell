...
code("import cellxgene_census, anndata as ad",
   "TFset=set(bb.loc[bb.is_tf==1,'gene'])",
   "adata=None; cell_means=None",
   "try:",
   "    with cellxgene_census.open_soma(census_version='stable') as census:",
   "        VF=(\"is_primary_data==True and tissue_general in \"",
   "            \"['blood','liver','heart','lung','brain','kidney','immune system']\")",
   "        obs=cellxgene_census.get_obs(census,'Homo sapiens',value_filter=VF,column_names=['soma_joinid','cell_type'])",
   "        obs=pd.DataFrame(obs)",
   "        ids=obs.groupby('cell_type',group_keys=False).apply(lambda d:d.sample(min(len(d),200),random_state=0))",
   ...
   "               obs_coords=ids['soma_joinid'].tolist(),",
   "               var_value_filter=\"feature_biotype=='gene'\")",
   "    adata.var_names=adata.var['feature_name'].astype(str)",
   "    import scipy.sparse as sp",
   "    Xd=adata.X; Xd=np.asarray(Xd.todense()) if sp.issparse(Xd) else np.asarray(Xd)",
   "    Xd=np.log1p(Xd)",
   "    df=pd.DataFrame(Xd,columns=adata.var_names); df['ct']=adata.obs['cell_type'].values",
   "    cell_means=df.groupby('ct').mean()            # cell_type x gene",
   "    cell_means.to_csv(f'{OUT}/celltype_expression.csv')",
   "    print('cell-type expression:',cell_means.shape)",
   "except Exception as e:",
   "    print('Model 2 census unavailable, will fall back to curated masters:',e)"),
code("masters={}",
