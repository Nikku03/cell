...
code("# [2b] fetch the raw data build_cell_complete.py needs (the big files are NOT in the repo).",
   ...
   "def _dl(url,p):",
   "    try:",
   "        urllib.request.urlretrieve(url,p); print('   ->',os.path.getsize(p)//1024,'KB'); return True",
   ...
   "        print('downloading Perturb-seq genome-wide pseudobulk (~375 MB) ...'); _dl('https://plus.figshare.com/ndownloader/files/35773217', psh)",
   ...
   "    for fn,env in [('perturbseq_rpe1_bulk.h5ad','PERTURBSEQ_RPE1_URL'),('perturbseq_norman.h5ad','PERTURBSEQ_NORMAN_URL')]:",
   "        u=os.environ.get(env,'')",
   "        if u and not (os.path.exists(f'{H}/{fn}') and os.path.getsize(f'{H}/{fn}')>1e7):",
   "            print('downloading',fn,'...'); urllib.request.urlretrieve(u, f'{H}/{fn}')",
