...
code("# [2b] fetch the raw data build_cell_complete.py needs (the big files are NOT in the repo).",
   ...
   "for fn in ['gene_compartment.json','collectri.tsv','reactome_human.txt','gene_info.gz','string_aliases.txt.gz','string_physical.txt.gz','hiv_interactions','human_gem.txt']:",
...
code("# build with hard failure if anything is missing (so we never silently ship the old committed HTML)",
   ...
   "for s in ['build_cell_complete.py','build_cell_app_complete.py']:",
   ...
   "print('FRESH cell_complete.html:', os.path.getsize(f'{OUT}/cell_complete.html')//1024,'KB',",
   "      '| rebuilt', round(time.time()-os.path.getmtime(f'{OUT}/cell_complete.html'),1),'s ago')",
   "if PROJ:",
   "    import shutil",
