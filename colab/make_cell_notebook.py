...
code("# build with hard failure if anything is missing (so we never silently ship the old committed HTML)",
   ...
   "if os.environ.get('RUN_CATPRED','0')=='1':   # OPTIONAL CatPred kcat/Km inference (needs a GPU runtime). OFF by default.",
   "    import subprocess as _cp",
   "    if not os.path.isdir('CatPred'): _cp.run(['git','clone','--depth','1','https://github.com/maranasgroup/CatPred.git'])",
   "    _cp.run([sys.executable,'-m','pip','install','-q','chemprop','fair-esm','rdkit-pypi'])",
   "    # our catpred_input.csv already has CatPred's required columns (SMILES, sequence, pdbpath). Verify the",
   "    # exact predict entrypoint against CatPred/README (demo_run.py) -- it may need --checkpoint / model args.",
   "    rci=_cp.run([sys.executable,'CatPred/scripts/demo_run.py','--input',f'{OUT}/catpred_input.csv','--output',f'{OUT}/catpred_predictions.csv'],capture_output=True,text=True); print(rci.stdout[-1200:]); print('ERR',rci.stderr[-800:])",
   "    _cp.run([sys.executable,'colab/compute_catpred_kinetics.py'])   # parse predictions -> catpred_kinetics.json",
