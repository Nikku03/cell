...
code("# [2b] fetch the raw data build_cell_complete.py needs (the big files are NOT in the repo).",
   ...
   "print(f'Drive restore: {n_raw} raw file(s) + {n_out} precomputed output(s); ARCHS4 read in place from expression_geo/')",
   "SRC={",
   ...
   "    # PaxDb protein abundance (quantitative 'how much' layer) + CORUM curated complexes",
   "    _get('paxdb_human.txt', os.environ.get('PAXDB_URL','https://pax-db.org/downloads/latest/datasets/9606/9606-WHOLE_ORGANISM-integrated.txt'))",
   "    _sp.run([sys.executable,'colab/compute_abundance.py'])                   # -> paxdb_abundance.json",
