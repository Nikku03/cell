...
cells=[
 ...
 code("DATASETS = "+repr([list(d) for d in DATASETS]),
    ...
    "        done=cnt*bs; el=max(time.time()-t0,1e-6); spd=done/el/1e6",
    "        if total>0:",
    "            pct=min(100,done*100/total); eta=(total-done)/max(done/el,1)",
    "            print(f'\\r    {fn}: {done/1e9:6.2f}/{total/1e9:.2f} GB  {pct:5.1f}%  {spd:5.1f} MB/s  ETA {eta/60:4.1f} min   ',end='',flush=True)",
    "        else:",
    "            print(f'\\r    {fn}: {done/1e6:7.0f} MB  {spd:5.1f} MB/s   ',end='',flush=True)",
    "    try:",
