...
<div id=top>
 ...
 <span class=btn id=mPerturb>Remove/Mutate</span><span class=btn hiv id=mHIV>Infect: HIV</span>
...
let mode='Explore',sel=-1,affected=null,hivOn=false;
...
function draw(){ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle='#080e16';ctx.fillRect(0,0,W,Hh);
 ...
 for(let i=0;i<N;i++){let X=POS[i*2],Y=POS[i*2+1],r=1.3+Math.min(G[i].ppi/45,2.4);
   let dim=(affected&&!affected.set.has(i)&&i!=sel)||(hivOn&&!hivHost[i]&&!affected&&mode=='Explore'&&false);
   let hl=affected&&affected.set.has(i);
   ctx.globalAlpha=dim?0.06:0.9;ctx.beginPath();ctx.arc(X,Y,hl?r+1:r,0,7);
   ctx.fillStyle=hl?affected.col(i):baseColor(i);ctx.fill();
...
// modes
function setMode(m){mode=m;affected=null;
...
document.getElementById('mHIV').onclick=()=>{hivOn=!hivOn;document.getElementById('mHIV').classList.toggle('on',hivOn);affected=null;if(hivOn){mode='Explore';hivReport();}else setMode('Explore');draw();};
document.getElementById('reset').onclick=()=>{sel=-1;affected=null;hivOn=false;document.getElementById('mHIV').classList.remove('on');setMode('Explore');fit();document.getElementById('info').innerHTML=welcome;draw();};
document.getElementById('q').addEventListener('change',e=>{let n=e.target.value.trim();if(idxByName[n]!==undefined)go(n);});
const welcome=`<h2>The cell</h2><div class=lg>${N} proteins in their compartments, wired by ${D.reg.length} regulatory + ${D.ppi.length} physical edges.<br><br>
 <b>Explore</b>: click any protein.<br><b>Processes</b>: colored by function.<br><b>Remove/Mutate</b>: knock out a protein &rarr; watch the cascade &amp; whether the cell survives.<br><b>Infect: HIV</b>: what HIV hijacks + its weak points.</div>
