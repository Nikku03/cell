...
def main():
    ...
    X = node_features(G)
    A = build_adj(D, len(G))
    # PPI is the headline and runs the HYBRID (fixed→ML injection); reg/sig run the standard ladder
    rows = [gnn.head_to_head(D, X, A, relation=r, epochs=EPOCHS, hybrid=(r == "ppi")) for r in ("ppi", "reg", "sig")]
    ...
    result = dict(relations=rows, n_relations=len(rows), learned_wins=wins,
                  ladder_ppi=dict(fixed=ppi["fixed_auc"], graphsage=ppi.get("sage_auc"),
                                  rgcn=ppi.get("rgcn_auc"), hybrid=ppi.get("hybrid_auc")),
                  headline_ppi=dict(fixed=ppi["fixed_auc"], learned=ppi["learned_auc"], delta=ppi["delta"]),
                  best_encoder="hybrid: R-GCN over [node features | fixed SIGN embedding] (ML takes input from fixed)",
                  device=rows[0]["device"], epochs=EPOCHS,
                  passed=bool(ppi["learned_auc"] > ppi["fixed_auc"] + 0.01 and wins >= 2),
                  detail="fixed SIGN vs GraphSAGE vs R-GCN vs hybrid(R-GCN+fixed-embedding); leakage-free split")
    json.dump(result, open(OUT / "cellgraph_gnn_validation.json", "w"), indent=2)
    print("=" * 78)
    print(f"LEARNED GNN vs FIXED — {'PASS (learned wins)' if result['passed'] else 'no improvement'}"
          f"  [{rows[0]['device']}, {EPOCHS} epochs]")
    print("=" * 78)
    print(f"  {'relation':8} {'fixed':>8} {'GraphSAGE':>10} {'R-GCN':>8} {'hybrid':>8} {'Δ(best-fixed)':>14}")
    for r in rows:
        mark = "  <- win" if r["learned_auc"] > r["fixed_auc"] + 0.005 else ""
        print(f"  {r['relation']:8} {r['fixed_auc']:>8.4f} {r.get('sage_auc', 0):>10.4f} "
              f"{r.get('rgcn_auc', 0):>8.4f} {(r.get('hybrid_auc') or 0):>8.4f} {r['delta']:>+14.4f}{mark}")
    print(f"\n  ladder (PPI): fixed {ppi['fixed_auc']} -> GraphSAGE {ppi.get('sage_auc')} -> "
          f"R-GCN {ppi.get('rgcn_auc')} -> hybrid {ppi.get('hybrid_auc')}  (fixed→ML injection)")
