...
md("# Cell-model ensemble — training notebook",
   ...
   "|---|---|---|---|",
   "| **1. gene embeddings + combiner** (this notebook) | load Geneformer, grab gene-embedding matrix + train tiny MLP | **CPU or free T4 is enough** | ~5-10 min |",
   "| 2. cell-type state (Tabula Sapiens ~500k cells) | run Geneformer over all cells | **T4** subset(50k)~30 min / **A100** full~45 min | hours on T4 |",
   "| 3. LoRA fine-tune | gradients through adapters | **A100 / L4** | hours |",
   "",
   "**Recommendation:** do Phase 1 on **free Colab (T4)** - it's basically CPU work. Only move to **A100 (Colab Pro+ / pay-as-you-go)** for Phase 2/3 when you scale to the full atlas. Set *Runtime -> Change runtime type -> T4 GPU*."),
...
md("## Phase 2 - cell-type state (the keystone). Needs Tabula Sapiens + GPU.",
