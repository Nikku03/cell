...
![Figure 1. The reproducibility floor of bacterial Tn-seq (binary and continuous), with a conservation baseline plotted against it.](outputs/kappa_floor_figure.png)
...
### 4.2 The calibrated abstention atlas (v4)

The deployable artifact follows from the negatives in §4.1: a calibrated predictor that *predicts where it can and quarantines where it cannot*. We assemble 10 evidence channels (3 positive-essential, 5 positive-non-essential including the only positive-dispensability signal not reducible to inverted conservation — within-species pangenome absence in sister strains — plus 2 ortholog-vote and dN/dS label-transfer channels) and 2 phenotype-required quarantine flags into a 5-tier system with explicit reason codes per gene.
...
![Figure 7. DEG pathogen conditional essentials vs unconditional essentials, conservation distributions. Statistically distinguishable (p = 4 × 10⁻³); the magnitude lives almost entirely in the niche-condition subset (see text).](outputs/orphan/deg_conditional_conservation.png)
