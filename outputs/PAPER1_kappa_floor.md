...
*(A)* Variance partition of the 17,950 per-pair Spearman values: technical 47.7% (unique), shared 4.3%, condition-mismatch 2.6% (unique), residual/irreducible 45.4% — nearly half of the apparent irreproducibility is library/technical quality, not biology. *(B)* The floor on the Spearman scale: raw 10–90% band, raw median floor (ρ = 0.38), and the matched-condition, high-quality ceiling (ρ ≈ 0.77); the arrow marks the headroom recovered by removing technical noise — the achievable ceiling is roughly twice the naïve median. A representative predictor claim (e.g. ρ = 0.65) sits within this recovered headroom, interpretable only against the ceiling. Generated by `paper1_figure2.py` (`outputs/paper1_figure2.png`).
