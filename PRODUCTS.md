...
- The **extrinsic axis** prefers the measured SKEMPI bind ΔΔG, and otherwise uses the trained `bind_ddg` predictor (structure-only physics+rotamer+charge, all 20 AAs) — held-out-by-complex on the **full SKEMPI set (277 complexes, 4,417 muts): r≈0.51 all-AA, 0.51 non-alanine, 0.51 alanine, hotspot AUC 0.77.** So it *predicts* binding effect for any mutation on a complex, not just measured ones, and does so **equally well across all substitution types** — the earlier 0.30 non-alanine (on a 42-complex subset) was **data-starvation, not intrinsic difficulty**; the gap vanishes at scale. **ESM does not help this axis** (measured: ESM-alone r=0.09) — it reads a single chain, is partner-blind, and scores the fold/fitness axis, which is orthogonal to binding. (Committed model is the 42-complex fallback; the full-SKEMPI model comes from `bind_ddg_colab.ipynb`.) The model is **joint single+multi**: it also predicts **multi-point mutations** (2+ substitutions) via `predict_multi` — a free scope-add at zero cost to single (held-out multi r≈0.38 / hotspot AUC 0.76).
