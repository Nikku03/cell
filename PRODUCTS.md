...
- The **extrinsic axis** prefers the measured SKEMPI bind ΔΔG, and otherwise uses the trained `bind_ddg` predictor (structure-only physics+rotamer+charge, all 20 AAs) — held-out-by-complex **r≈0.46 all-AA / 0.30 non-alanine, hotspot AUC 0.79**. So it now *predicts* binding effect for any mutation on a complex, not just measured ones. The honest limit is the non-alanine number (~0.30): general substitutions are hard and SKEMPI is ~85% alanine. **ESM does not help this axis** (measured: ESM-alone r=0.09) — it reads a single chain, is partner-blind, and scores the fold/fitness axis, which is orthogonal to binding.
